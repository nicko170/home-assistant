...
class AppleTVManager(DeviceListener):
    ...
    async def _scan(self) -> AppleTV | None:
        ...
        # PATCHED (2026-08-29): HA's shared AsyncZeroconf is deliberately NOT
        # passed to pyatv. Handing that instance to pyatv.scan makes it return
        # zero devices, whatever the instance is configured with. Measured in
        # this container using HA's own identifier and protocol filters:
        #
        #     no aiozc (pyatv makes its own)      found=1
        #     Default + V4Only  (HA's config)     found=0
        #     per-IP 192.168.139.2                found=0
        #     InterfaceChoice.All                 found=0
        #
        # Upstream the effect is that this scan never returns anything, so the
        # connect loop retries forever and the entity sits at "off". apple_tv
        # reports a lost connection identically to a powered-off device, so
        # anything keying off that state reads it as the Apple TV being
        # switched off - which is how an automation ended up killing the room
        # mid-film on 2026-08-29.
        #
        # Dropping the argument lets pyatv build its own short-lived instance.
        atvs = await scan(
            hass.loop,
